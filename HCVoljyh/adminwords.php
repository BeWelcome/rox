...
//		echo "<br><a href=adminwords.php?code=",$_SESSION['TranslationArray'][$ii],"&IdLanguage=0>edit</a>" ;
		...
		if (isset($rr->idword)) {
		  if (strtotime($rword->updated)>strtotime($rr->updated)) { // if obsolete
		    ...
		    if (isset($rr->Sentence)) echo $rr->Sentence ;
...
if ((isset($_POST['DOACTION']))and($_POST['DOACTION']=='Find')) {
  ...
  if ($_POST['Sentence']!="") {
	...
	while ($rr=mysql_fetch_object($qry)) {
	  if ($countfind==0) echo "<tr align=left><th>id</th><th>code</th><th>Sentence</th><th>langue</th>\n" ;
	  $countfind++ ;
	...
	echo "</table>\n" ;
  if ($countfind==0) echo "<h3><font color=red>",$where," Not found</font></h3>\n" ;
