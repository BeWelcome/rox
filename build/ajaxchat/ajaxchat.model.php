...
class AjaxchatModel extends RoxModelBase
{
    ...
    public function __construct($MyIdRoom=1,$Title="",$Description="") {
     ...
     $rr=$this->singleLookup("select * from chat_rooms_members where IdRoom=".$this->room->id." and IdMember=".$IdMember) ;
		...
		function RemovePeopleFromRoom($IdRoom=0) {
				$room=$this->singleLookup("select * from chat_rooms where IdRoom=".$IdRoom) ;
      	MOD_log::get()->write("RemovePeopleFromRoom room #".$this->IdRoom.' '.$room->RoomTitle ,"chat") ;
      	...
      	MOD_log::get()->write("RemovePeopleFromRoom done (".$qq->affectedRows()." people removed)" ,"chat") ;
		}
		
		function CleanRoom($IdRoom=0) {
				$room=$this->singleLookup("select * from chat_rooms where id=".$IdRoom) ;
				if (!MOD_right::get()->HasRight("Chat","CleanRoom") and ($room->IdRoomOwner!=$_SESSION["IdMember"])) {
					$ss="You are not allowed to clean Room #".$IdRoom ;
	      	MOD_log::get()->write($ss ,"chat") ;
					die($ss) ;
					return ;
				}
      	MOD_log::get()->write("Cleaning room #".$this->IdRoom.' '.$room->RoomTitle ,"chat") ;
      	...
      	MOD_log::get()->write("Cleaning done (".$qq->affectedRows()." messages deleted)" ,"chat") ;
		}
...
FROM
    ...
    members
WHERE
    chat_messages.IdAuthor   = members.id     AND
...
WHERE
    chat_messages.IdRoom = $this->IdRoom  AND
    members.id                = chat_messages.IdAuthor      AND
    ...
    public function createMessageInRoom($IdAuthor, $text) {
        ...
        $ss="INSERT INTO chat_messages SET  IdRoom = ".$this->IdRoom ;
				$ss=$ss.",IdAuthor   = $IdAuthor, text= '$text', created= NOW(),  updated= NOW() " ;
//				$ss=$ss.",IdAuthor   = $IdAuthor, text= '\$this->room->id=$this->IdRoom', created= NOW(),  updated= NOW() " ;
						
						
 				$result = $this->dao->query($ss);
				if (!$result) {
      		MOD_log::get()->write("Failed to insert message in room #".$this->IdRoom." $ss" ,"chat") ; 				
	   			throw new PException('Failed to insert mesage in room #'.$this->IdRoom);
				}
				
      	MOD_log::get()->write("Has post  Message_id #".$result->insertId()." in room #".$this->IdRoom ,"chat") ; 				
        
// Mark that a member activity in the room (since he retrieves messages)
        $ss="REPLACE into chat_rooms_members (IdRoom,IdMember,LastWrite) values(".$this->IdRoom.",".$_SESSION["IdMember"].",now() )" ;
				
 				$result = $this->dao->query($ss);
				if (!$result) {
	   			throw new PException('Failed to update the write activity of member '.$_SESSION["IdMember"].' in room #'.$this->IdRoom);
				}
