...
`blog_category_id` = '.$this->dao->nextId('blog_categories').',
    ...
    */
    public function suggestTags($search)
    {
        ...
        if ($number_words && $this->_session->has( 'prev_tag_content' ) && $this->_session->get('prev_tag_content')) {
            $search_for = false;
            $pos = false;
            for ($i = 0; $i < $number_words; $i++) {
                if (isset($words[$i]) && (!$this->_session->has( 'prev_tag_content[' . $i . ']') || $words[$i] != $this->_session->get('prev_tag_content'][$i))) {
                    $search_for = $words[$i];
                    $pos = $i;
                }
            }
        ...
        if ($search_for) {
            ...
            $this->_session->set( 'prev_tag_content', $words )
