    ...
    <div class="forumsmessage">
        <p class="forumstime">
            ...
            if ($can_edit_own && $User && $post->user_id == $User->getId()) {
                echo ' [<a href="forums/edit/m'.$post->postid.'">'.$words->getBuffered('forum_EditTranslate').'</a>]';
            }
...
//                echo ' <b>'.$post->title.'</b> &mdash; <a href="'.ForumsView::postURL($post).'">'.$words->getFormatted('search_topic_href').'</a>';
        ...
        <p><?php 
		 // echo $post->message;
		 // TODO: some clever logic to decide which trad to show where
		 if (isset($post->active_trad)) {
		     echo '<p><strong>Active trad sentence ('.$post->active_trad->language_code.')</strong></p>';
             echo '<p>'.$post->active_trad->Sentence.'</p>'; 
		 } else {
		 ...
		 if (!isset($post->trads)) {
		     // whatever
		 } else foreach ($post->trads as $trad) {
		     echo '<p><strong>'.$trad->language_code.'</strong></p>';
		     echo '<p>'.$trad->Sentence.'</p>'; 
		 }
		 ?></p>
 	     </div> <!-- forumsmessage -->
