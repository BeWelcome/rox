...
// Retrieve the tags description
	...
	*/
    ...
    private function prepare_notification($IdPost,$Type) {
        ...
        $query = sprintf("select forums_posts.threadid as IdThread,forums_threads.IdGroup as IdGroup,PostVilibility,PostDeleted,ThreadVisibility,ThreadDeleted from forums_posts,forums_threads where forums_posts.IdThread=forums_threads.id and forums_posts.postid=%d",$IdPost) ;
        $s = $this->dao->query($query);
        if (!$s) {
