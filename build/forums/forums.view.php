...
//        PVars::getObj('page')->title = $this->words->getBuffered('Forum').' - BeWelcome - Last Post in Catgegories';
    ...
    public function getVisibilityCheckbox($visibility, $highestVisibility, $IdGroup, $newTopic) {
        ...
        $out = '<div class="o-checkbox"><input type="checkbox" class="o-checkbox__input" name="' . $name . '" id="' . $name . '" value="GroupOnly"';
        if ($visibility == 'GroupOnly') {
            $out .= ' checked="checked" ';
        }
        $out .= '/> <label for="' . $name . '" class="o-checkbox__label">' . $words->get($word) . '</label></div>';
        return $out;
    }
