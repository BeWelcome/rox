...
<div class="l-forum-single-post <?php echo $styles[$cnt % 2]; ?>">
    ...
    <div class="c-single-post-user_info">
        ...
        <div class="d-flex flex-row text-break">
            <img class="profileimg avatar-48 mr-1" width="48" height="48" src="/members/avatar/<?php echo($post->OwnerUsername); ?>/48">
            <div class="small">
    ...
    <div class="c-single-post-post_info">
        <div class="d-flex flex-column flex-sm-row small">
            <?php
                echo '<div class="mr-1"><span><i class="fa fa-comment fa-w-16 mr-1" title="' . $words->getFormatted('posted'); ?>"></i><?php echo date($words->getBuffered('DateHHMMShortFormat'), ServerToLocalDateTime($post->posttime, $this->getSession())) . '</span></div>';
                $max = 0;
                if (!empty($post->Trad)) {
                    $max = count($post->Trad);
                }
                ...
                echo $words->flushBuffer() . '<div class="mr-1"><i class="fa fa-eye fa-w-16 mr-1" title="' . $words->getFormatted("forum_label_visibility") . '"></i>' . $words->getFormatted("forum_edit_vis_" . $post->PostVisibility) . '</div>';
                ?>
        </div>
    </div>
    ...
    <div class="c-single-post-permalink">
            <?php
                if (isset($post->IdGroup) && $post->IdGroup != 0) {
                    echo '<small><a href="group/' .$post->IdGroup . '/forum/s' . $post->threadid . '/#post' . $post->postid . '"><i class="fa fa-link"></i> ' . $words->get('ForumPermalink') . '</a></small>';
                } else {
                    echo '<small><a href="forums/s' . $post->threadid . '/#post' . $post->postid . '"><i class="fa fa-link"></i> ' . $words->get('ForumPermalink') . '</a></small>';
                }
            ?>
    </div>
