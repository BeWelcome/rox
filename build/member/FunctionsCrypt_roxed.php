...
function NewInsertInCrypted($ss,$TableColumn,$IdRecord, $_IdMember = "", $IsCrypted = "crypted") {
   ...
   $ssM=GetCryptM($ss,$IsCrypted);
	$sQuery = "insert into ".$_SYSHCVOL['Crypted']."cryptedfields(AdminCryptedValue,MemberCryptedValue,IdMember,IsCrypted,TableColumn,IdRecord) values(\"" . $ssA . "\",\"" . $ssM . "\"," . $IdMember . ",\"" . $IsCrypted . "\",\"" . $TableColumn . "\",".$IdRecord.")";
    $this->dao->exec($sQuery);
...
function MemberCrypt($IdCrypt) {
	global $_SYSHCVOL; // use global vars
	$IdMember = $_SESSION['IdMember'];
    $sQuery = "select MemberCryptedValue from ".$_SYSHCVOL['Crypted']."cryptedfields where IdMember=" . $IdMember . " and id=" . $IdCrypt;
    $s = $this->dao->query($sQuery);
	$rr = $s->fetch(PDB::FETCH_OBJ);
   $ssM=GetCryptM($rr->MemberCryptedValue);
	$sQuery = "update ".$_SYSHCVOL['Crypted']."cryptedfields set IsCrypted='crypted',MemberCryptedValue='".$ssM."' where IsCrypted='not crypted' and IdMember=" . $IdMember . " and id=" . $IdCrypt;
    $this->dao->exec($sQuery);
} // end of MemberCrypt
...
function MemberDecrypt($IdCrypt = 0) {
   ...
   $ssM=GetDeCryptM($rr->MemberCryptedValue);
	$sQuery = "update ".$_SYSHCVOL['Crypted']."cryptedfields set IsCrypted='not crypted',MemberCryptedValue='".$ssM."' where IsCrypted='crypted' and IdMember=" . $IdMember . " and id=" . $IdCrypt;
    $this->dao->exec($sQuery);
...
function IsCrypted($IdCrypt) {
    ...
    $sQuery = "select SQL_CACHE * from ".$_SYSHCVOL['Crypted']."cryptedfields where id=" . $IdCrypt;
    $s = $this->dao->query($sQuery);
	$rr = $s->fetch(PDB::FETCH_OBJ);
	switch ($rr->IsCrypted) {
		case "not crypted" :
