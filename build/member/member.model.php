 ...
 * @license http://www.gnu.org/licenses/gpl.html GNU General Public License (GPL)
  * @version $Id: member.model.php 217 2007-03-03 10:21:10Z lupochen $
 */
class Member extends PAppModel
{
     ...
     */
    const HANDLE_PREGEXP = '%^[a-z][a-z0-9\-]{3,}$%i';
...
public function getmembersgroups($IdMember) {
		$query = "select SQL_CACHE membersgroups.Comment as Comment,groups.Name as Name,groups.id as IdGroup from groups,membersgroups where membersgroups.IdGroup=groups.id and membersgroups.Status='In' and membersgroups.IdMember=" . $IdMember;
		$s = $this->dao->query($query);
		if( !$s) {
			throw new PException('Could not retrieve Groups!');
		}
...
function prepareProfileHeader($handle,$wherestatus="",$photorank=0) {
    ...
    // TO DO: make Cryptation work for Rox and read member's fullname
	// Load full name
	// $m->FullName = fFullName($m);

	// Load Address data
	$sQuery = "select SQL_CACHE * from addresses where IdMember=" . $m->id . " and Rank=0 limit 1";
    $s = $this->dao->query($sQuery);
	$rr = $s->fetch(PDB::FETCH_OBJ);
	/*if (isset ($rr->id)) {
	    $m->Address = PublicReadCrypted($rr->HouseNumber, "*") . " " . PublicReadCrypted($rr->StreetName, $words->getFormatted("MemberDontShowStreetName"));
    ...
    if ($User = APP_User::login()) {
	   ...
	   $qry = mysql_query($str);
	   while ($rr = mysql_fetch_object($qry)) {
		  if (($User = APP_User::login()) and (!IsPublic($rr->IdMember))) continue; // Skip non public profile is is not logged

		  $rr->Comment=FindTrad($rr->Comment,true);
    	$sQuery = "select SQL_CACHE * from membersphotos where IdMember=" . $rr->IdRelation . " and SortOrder=0";
        $s = $this->dao->query($sQuery);
    	$photo = $s->fetch(PDB::FETCH_OBJ);
	   ...
	   // check if the member is in mycontacts
    	$sQuery = "select SQL_CACHE * from mycontacts where IdMember=".$_SESSION["IdMember"]." and IdContact=".$IdMember;
        $s = $this->dao->query($sQuery);
    	$rr = $s->fetch(PDB::FETCH_OBJ);
	   if (isset($rr->id)) {
	   	  $m->IdContact=$rr->id; // The note id
	   ...
	   // check if wether this profile has a special relation with teh current member
    	$sQuery = "select SQL_CACHE * from specialrelations where IdOwner=".$_SESSION["IdMember"]." and IdRelation=".$IdMember;
        $s = $this->dao->query($sQuery);
    	$rr = $s->fetch(PDB::FETCH_OBJ);
	   if (isset($rr->IdRelation)) {
	   	  $m->IdRelation=$rr->IdRelation; // there is no special relation
    ...
    $sQuery = "select count(*) as cnt from forums_posts where IdWriter=".$IdMember;
    $s = $this->dao->query($sQuery);
    $rr = $s->fetch(PDB::FETCH_OBJ);
