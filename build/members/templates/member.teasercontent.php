<div class="row">
    <div class="col-12 text-center text-md-left">
        <h2 class="m-0 d-inline"><strong>
                <?php if ($this->passedAway == 'PassedAway') {
                    echo $words->get('ProfileInMemoriam', $member->Username);
                } else {
                    echo $member->Username;
                } ?>
            </strong>

            <?php if (!$this->passedAway) : ?>
                <?php
                $icons = [];
                if (strstr((string) $member->TypicOffer, "CanHostWeelChair")) {
                    $icons[] = '<img src="images/icons/wheelchairblue.png" ' .
                        'class="mb-2" ' .
                        'alt="' . $words->getSilent('wheelchair') . '" ' .
                        'title="' . $words->getSilent('CanHostWheelChairYes') . '" />';
                }

                $icons[] = '<img src="images/icons/' . $member->Accomodation . '.png"' .
                    ' class="mb-2"' .
                    ' alt="' . $words->getSilent($member->Accomodation) . '"' .
                    ' title="' . $words->getSilent('CanOffer' . $member->Accomodation) . '" />';

                for ($ii = 0; $ii < count($icons); $ii++) {
                    echo $icons[$ii];
                }
                ?>
                <?= $words->flushBuffer() ?>
            <?php endif; ?>
        </h2>
    </div>
</div>
<div class="row align-items-center">
    <div class="col-12 col-sm-6">
        <?php
        $picture_url = 'members/avatar/' . $member->Username;
        ?>
        <div class="text-center text-md-left">
            <div class="d-md-none u:w-full u:relative u:pb-[100%] u:m-auto">
                <div class="u:absolute u:left-0 u:top-0 u:w-full u:h-full">
            <?php if ($this->useLightbox) { ?>
                        <a href="<?= $picture_url ?>/original" title="<?= $words->get('profile.picture.title'); ?>" class="js-profile-picture" data-toggle="lightbox" data-type="image">
                            <img class="u:rounded-full u:w-full u:h-full u:object-cover js-profile-picture"
                                 src="<?= $picture_url ?>/320" alt="<?= $words->get('profile.picture.title'); ?>"/>
                        </a>
            <?php } else { ?>
                        <a href="members/<?= $member->Username; ?>" title="<?= $words->get('profile.picture.title'); ?>">
                            <img class="u:rounded-full u:w-full u:h-full u:object-cover js-profile-picture"
                                 src="<?= $picture_url ?>/320" alt="<?= $words->get('profile.picture.title'); ?>">
                        </a>
            <?php } ?>
                </div>
            </div>

            <?php if ($member->name() != '') {
                echo "<h3>" . $member->name() . "</h3>";
            } ?> <!-- user's name -->

            <h5>
                <?php
                // The "Hong Kong solution": Only display and link country.
                if ($member->region() == '' && $member->city() == $member->country()) :
                    ?>
                    <strong><a
                            href="places/<?php echo urlencode((string) $member->country()) . "/" . urlencode((string) $member->countrycode()); ?>"><?php echo $member->country(); ?></a></strong>
                <?php
                // In case of missing parent in Geonames DB: Only display city and country. Don't link city.
                elseif ($member->region() == '') :
                    ?>
                    <strong><?php echo urlencode((string) $member->city()); ?></strong>, <a href="places/<?php
                echo urlencode((string) $member->country()) . "/" . urlencode((string) $member->countryCode()); ?>"><?php echo $member->country(); ?></a>
                <?php
                // For every other city display normal path. Don't show region if it has the same name as city.
                else :
                    ?>
                    <strong><a
                        href="places/<?php echo urlencode((string) $member->country()) . "/" . urlencode((string) $member->countrycode())
                            . "/" . urlencode((string) $member->region()) . "/" . urlencode((string) $member->regioncode()) . "/"
                            . urlencode((string) $member->city) . "/" . $member->IdCity; ?>"><?php echo $member->city(); ?></a></strong><?php if ($member->region() != $member->city()) : ?>,
                    <a
                    href="places/<?php echo urlencode((string) $member->country()) . "/" . urlencode((string) $member->countryCode()) . "/"
                        . urlencode((string) $member->region()) . "/" . urlencode((string) $member->regioncode()); ?>"><?php echo $member->region(); ?></a><?php endif; ?>,
                    <a href="places/<?php echo urlencode((string) $member->country()) . "/" . urlencode((string) $member->countryCode()); ?>"><?php echo $member->country(); ?></a>
                <?php
                endif;
                ?>
            </h5><!-- location -->

            <div class="o-form-group form-inline small"><?php echo $this->statusForm($member); ?></div>
        </div>
    </div>
    <div class="col-12 col-sm-6 teaser-border text-center text-md-left">

        <?php if ($occupation != null) {
            echo '<p class="h5">' . $occupation . '</p>';
        } ?>
        <!-- occupation -->

        <p class="mb-2">
            <?php
            echo $agestr;
            $strGender = MOD_layoutbits::getGenderTranslated($member->Gender, $member->HideGender, false);
            if (!empty($strGender)) {
                echo ', ' . $strGender;
            }
            echo '<br>';
            ?>
            <?php if ($this->loggedInMember) : ?>
                <?php echo '<span class="font-weight-bold">' . $words->get('profile.joined') . ' </span>';
                if (strtotime((string) $member->created) > strtotime('-1 week')) {
                    echo $words->get("LastLoginPrivacy");
                } else {
                    echo $layoutbits->ago(strtotime((string) $member->created));
                }
                echo $this->memberSinceDate($member);
                echo '<br><span class="font-weight-bold">' . $words->get('profile.last.activity') . ' </span>';
                $lastLogin = (null == $member->LastLogin) ? $member->created: $member->LastLogin;
                if (strtotime((string) $lastLogin) > strtotime('-1 week')) {
                    echo $words->get("LastLoginPrivacy");
                } else {
                    echo $layoutbits->ago(strtotime((string) $lastLogin));
                }
                echo $this->lastLoginDate($lastLogin);
?>
            <?php endif; ?>
        </p>
    </div>
</div> <!-- profile header -->

<?php
if ($this::class == 'EditMyProfilePage' || $this::class == 'EditProfilePage') $urlstring = 'editmyprofile';
if ($this::class == 'ProfilePage' || $this::class == 'MyProfilePage') $urlstring = 'members/' . $member->Username;
if (isset($urlstring)) { ?>
    <div class="row">
        <div class="col-12">
            <?php require 'profileversion.php'; ?>
        </div>
    </div>
<?php } ?>
