...
// no, not for now
class PreviousversionController extends RoxControllerBase
{
    ...
    public function index($args=false)
    {
        ...
        $model = new PreviousversionModel;
...
//        print_r($args->post);
		if (!MOD_right::get()->HasRight("ContactLocation")) {
			...
			switch (isset($request[1]) ? $request[1] : false) {
				case 'choose':
					$data=$model->FindRes($args->post) ;
//					print_r( $args->post) ;
					$page = new PreviousversionPage("","showres",$data);
					break ;
				case 'loadres':
					$data=$model->LoadRes($args->post) ;
//					print_r( $args->post) ;
					$page = new PreviousversionPage("","loadres",$data);
					break ;
				...
				case '':
					...
					$page = new PreviousversionPage("","choose"); // Without error
					break;
			}
