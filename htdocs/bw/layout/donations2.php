...
function DisplayDonate($TDonation,$Message="") {
  ...
  $_SESSION["PaypalBW_key"]=crc32(rand(50,50000)) ;
  LogStr("Showing donation page with key  #".$_SESSION["PaypalBW_key"],"Donation") ; // This is not a security trick but just to find back the donator when he will come back
...
<?php
  echo "<input type=\"hidden\" name=\"tx\" value=\"".$_SESSION["PaypalBW_key"]."\">" ;
?>
<select  name="amount">
<option value="10.00" selected>5 €</option>
...
<input type="hidden" name="return" value="http://www.bewelcome.org/bw/donations2.php?action=done">
<input type="hidden" name="cancel_return" value="http://www.bewelcome.org/bw/donations2.php?action=cancel">
