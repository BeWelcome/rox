...
class Swift_Connection_SMTP extends Swift_ConnectionBase
{
	...
	{
		if (!$this->handle) throw new Swift_Connection_Exception("The SMTP connection is not alive and cannot be read from.");
		$ret = "";
		$line = 0;
	...
	{
		if (!$this->handle) throw new Swift_Connection_Exception("The SMTP connection is not alive and cannot be written to.");
		if (!@fwrite($this->handle, $command . $end) && !empty($command)) throw new Swift_Connection_Exception("The SMTP connection did not allow the command '" . $command . "' to be sent.");
	}
	/**
	...
	public function stop()
	{
		if ($this->handle)
		{
			...
			{
				throw new Swift_Connection_Exception("The SMTP connection could not be closed for an unknown reason.");
			}
			$this->handle = null;
		}
	}
