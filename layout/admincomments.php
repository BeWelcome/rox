...
function ShowList($TData) {
	...
	for ($iData = 0; $iData < $maxTData; $iData++) {
		...
		if ($c->AdminComment != "Checked")
			echo "<a href=\"admincomments.php?IdComment=", $c->id, "&action=Checked\">Checked</a><br><br>\n";
		if (($c->AdminComment != "Checked") and (HasRight("Comments", "AdminComment")))
			echo "<a href=\"admincomments.php?IdComment=", $c->id, "&action=AdminCommentMustCheck\">Admin Comment Must Check</a><br><br>\n";
		if (($c->AdminComment != "Checked") and (HasRight("Comments", "AdminAbuser")))
			echo "<a href=\"admincomments.php?IdComment=", $c->id, "&action=AdminAbuserMustCheck\">Admin Abuser Must Check</a><br><br>\n";
		if (($c->AdminComment != "Checked") and (HasRight("Comments", "DeleteComment")))
			echo "<a href=\"admincomments.php?IdComment=", $c->id, "&action=del\" onclick=\"return('Confirm delete ?');\">del</a><br><br>\n";
...
function DisplayAdminComments($TData, $lastaction = "") {
	...
	if (HasRight("Comments", "AdminAbuser"))
		echo " <a href=\"admincomments.php?action=AdminAbuser\">Comments to check by Admin Abuser</a>";
