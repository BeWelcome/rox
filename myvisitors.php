...
$str .= " from cities,countries,regions,recentvisits,members left join membersphotos on membersphotos.IdMember=members.id and membersphotos.SortOrder=0 where cities.IdRegion=regions.id and countries.id=regions.IdCountry and cities.id=members.IdCity and status='Active' and members.id=recentvisits.IdVisitor and recentvisits.IdMember=" . $IdMember . " and members.status='Active' GROUP BY members.id order by recentvisits.created desc";
$qry = sql_query($str);
