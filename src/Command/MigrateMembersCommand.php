...
class MigrateMembersCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        try {
            ...
            $this->connection->executeStatement('TRUNCATE `address`; TRUNCATE `member`; TRUNCATE `member_translations`; TRUNCATE `member_language_level`; TRUNCATE `relation`;');
            gc_enable();
            ...
            for ($i = 0; $i < $countOfMembers; $i += $batchSize) {
                ...
                foreach ($members as $member) {
                    ...
                    $statement->bindValue('member', $memberId);
                    if (null === $city) {
                    ...
                    foreach ($languageLevels as $languageLevel) {
                        ...
                        $language = $languages[$languageLevel['IdLanguage']] ?? null;
                        if (null === $language) {
                            $progressBar->setMessage($member['Username'], 'error');
                    ...
                    $statement = $this->connection->prepare("
                    select 
                        mt.Sentence as comment, sr.* 
                    from 
                        specialrelations sr, memberstrads mt 
                    where 
                        sr.IdOwner = :memberId 
                        and sr.IdOwner= mt.IdOwner 
                          and mt.IdLanguage = 0 
                          and sr.Comment = mt.idtrad 
                          and mt.TableColumn = 'specialrelations.comment'
                ");
                    ...
                    foreach ($specialRelations as $specialRelation) {
                        ...
                        $statement->bindValue('confirmed', $languageLevel['confirmed'] ?? 0);
                        $statement->bindValue('owner_id', $memberId);
                        $statement->bindValue('relation_id', $specialRelation['IdRelation']);
                        try {
                            $statement->executeStatement();
                        } catch (Exception $e) {
                            $progressBar->setMessage($member['Username'], 'error');
