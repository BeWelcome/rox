...
class BlogController extends PAppController {
    ...
    private function _validateVars(&$vars) 
    {
        ...
        // check text
        if (!isset($vars['txt']) || empty($vars['txt'])) {
            $errors[] = 'text';
        }
        // check category
        ...
        if (isset($vars['tr']) && strcmp($vars['tr'],'')!=0 && !$this->isUserTrip(APP_User::get()->getId(), $vars['tr'])) {
            $errors[] = 'trip';
        }
        // geonames
    ...
    public function createProcess() 
    {
        if (PPostHandler::isHandling()) {
            ...
            $trip = (isset($vars['tri']) && strcmp($vars['tri'],'')!=0) ? (int)$vars['tri'] : false;
    ...
    public function editProcess()
    {
        if (PPostHandler::isHandling()) {
            ...
            $tripId = (isset($vars['tri']) && strcmp($vars['tri'],'')!=0) ? (int)$vars['tri'] : false;
