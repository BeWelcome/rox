...
<h2>
    <a href="user/<?php echo $currUser->getHandle(); ?>">
        <img src="user/avatar/<?=$currUser->getHandle()?>?xs=1" alt="<?=$currUser->getHandle()?>" class="l" style="margin:0 10px 0 0"/> <?=$currUser->getHandle()?></a>
<?php
if ($countrycode) {
?>        
...
?>" id="user-leftnav">
    <ul>
        <li><a href="user/friends"><?=$navText['friends']?></a></li>
        <li><a href="user/settings"><?=$navText['settings']?></a></li>
        <li><a href="message/inbox"><?=$navText['inbox']?></a></li>
    </ul>
<p>
